...
function retrievalSignals(caseRow, retrievedTopK, strategy) {
  ...
  // ─── "fullcontext": entire corpus is in the prompt; no retrieval ─────────
  // Sufficiency is 1 by construction (every fact is in context). Precision
  // and noise are not measurable. factCoverage marks every fact FULL so the
  // viewer can show a coherent drill-in.
  if (strategy === "fullcontext") {
    return {
      strategy,
      k: 0,
      taskHits: expectedTasks.map((t) => ({ task: t, hit: true, rank: 0 })),
      sufficiency: contentFacts.length === 0 ? null : 1,
      sufficiencyPartial: contentFacts.length === 0 ? null : 1,
      ...
      chunkPrecision: null,
      metadataFactsCount,
      _flags: { anyTaskHit: true, allTasksHit: true, fullcontext: true },
    };
  }
  ...
  // ─── "single": task-as-chunk; precision/noise measured at task grain ─────
  if (strategy === "single") {
    const taskPrecision = k === 0 ? null : chunksFromExpectedTasks / k;
    return {
      strategy,
      k,
      taskHits,
      ...
      precisionGrain: "task", // human-readable label for viewers
      noise: taskLevelNoise,
      noiseGrain: "task",
      // Back-compat alias (will be removed once viewers no longer reference it)
      chunkPrecision: null,
      metadataFactsCount,
  ...
  return {
    ...
    factCoverage,
    precision: chunkPrecision,
    precisionGrain: "chunk",
...
function classifyMiss(signals, caseRow, answerCorrect) {
  // Case-type labels take precedence over strategy. F1/F2/F3 should
  // keep their special labels (metadata_only / unanswerable / open_ended)
  // even on fullcontext rows — those aren't retrieval-vs-generation calls.
  ...
  if (signals === null || signals?._flags?.fullcontext) {
    // fullcontext on a content-fact case — pure generation evaluation
    return answerCorrect ? "success" : "generation_miss";
  }
