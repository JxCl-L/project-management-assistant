...
const a = JSON.parse(fs.readFileSync(path.join(ROOT, "results", `rag-quality-${tagA}.json`), "utf8"));
const b = JSON.parse(fs.readFileSync(path.join(ROOT, "results", `rag-quality-${tagB}.json`), "utf8"));
const cases = JSON.parse(fs.readFileSync(path.join(ROOT, "rag-eval-cases.json"), "utf8"));
const resultsA = JSON.parse(fs.readFileSync(path.join(ROOT, "results", `rag-eval-results-${tagA}.json`), "utf8"));
const resultsB = JSON.parse(fs.readFileSync(path.join(ROOT, "results", `rag-eval-results-${tagB}.json`), "utf8"));
const outPath = path.join(ROOT, "viewers", `rag-quality-compare-${tagA}-vs-${tagB}.html`);
...
for (const c of comparable) {
  ...
  drillData[c.k] = {
    a: c.ra ? { diagnosis: c.ra.diagnosis, mark: c.ra.answerMark, sufficiency: c.ra.retrievalSufficiency, sufficiencyPartial: c.ra.retrievalSufficiencyWithPartial, precision: c.ra.precision ?? c.ra.chunkPrecision, precisionGrain: c.ra.precisionGrain, noise: c.ra.retrievalNoise, noiseGrain: c.ra.noiseGrain, taskHits: c.ra.taskHits, factCoverage: c.ra.factCoverage, answer: answerA.get(cellK) ?? "", verdict: c.ra.reportVerdict } : null,
    b: c.rb ? { diagnosis: c.rb.diagnosis, mark: c.rb.answerMark, sufficiency: c.rb.retrievalSufficiency, sufficiencyPartial: c.rb.retrievalSufficiencyWithPartial, precision: c.rb.precision ?? c.rb.chunkPrecision, precisionGrain: c.rb.precisionGrain, noise: c.rb.retrievalNoise, noiseGrain: c.rb.noiseGrain, taskHits: c.rb.taskHits, factCoverage: c.rb.factCoverage, answer: answerB.get(cellK) ?? "", verdict: c.rb.reportVerdict } : null,
    meta: { caseId, strategy, category: (c.ra ?? c.rb).category, status: c.status, query: qt.query || "", groundTruth: qt.groundTruth || "", findings: c.ra?.reportFindings ?? c.rb?.reportFindings ?? null },
  };
}
    ...
    return \`
      <div class="side \${sideClass}">
        ...
        <dl>
          ...
          <dt>\${(r.precisionGrain ?? 'chunk')} precision</dt><dd>\${pct(r.precision)}</dd>
          <dt>\${(r.noiseGrain ?? 'chunk')} noise</dt><dd>\${pct(r.noise)}</dd>
        </dl>
      </div>
    \`;
  }
    ...
    content.innerHTML = \`
      ...
      <div class="qa-block">
        ...
        <div class="answer-side side-a">
          ...
          \${d.a?.verdict ? \`<div style="margin-top:8px;padding:6px 10px;background:#f8fafc;border-left:3px solid #cbd5e1;border-radius:0 4px 4px 0;font-size:11.5px;color:#334155;line-height:1.5;"><b>Curated verdict (\${escape(d.a.verdict.mark)})</b> · \${escape(d.a.verdict.text)}</div>\` : ''}
        </div>
        <div class="answer-side side-b">
          ...
          \${d.b?.verdict ? \`<div style="margin-top:8px;padding:6px 10px;background:#f8fafc;border-left:3px solid #cbd5e1;border-radius:0 4px 4px 0;font-size:11.5px;color:#334155;line-height:1.5;"><b>Curated verdict (\${escape(d.b.verdict.mark)})</b> · \${escape(d.b.verdict.text)}</div>\` : ''}
        </div>
      </div>

      \${d.meta.findings ? \`
        <div style="margin-bottom:14px;">
          <h3 style="font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:#475569;margin:14px 0 8px;">Case-level findings (shared across strategies)</h3>
          <div style="font-size:11.5px;color:#334155;line-height:1.5;background:#fefce8;border-left:3px solid #fde047;padding:8px 12px;border-radius:0 4px 4px 0;white-space:pre-wrap;">\${escape(d.meta.findings)}</div>
        </div>
      \` : ''}

      <h3 style="font-size:11px;text-transform:uppercase;letter-spacing:0.05em;color:#475569;margin:14px 0 8px;">Retrieval signals <span style="font-weight:400;text-transform:none;letter-spacing:0;color:#94a3b8;font-size:10.5px;">(based on retrieved chunks)</span></h3>
