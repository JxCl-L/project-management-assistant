...
// scripts/analyzers/<this file> · inputs from scripts/ root · outputs split:
//   results JSON → scripts/results/
//   reports MD   → scripts/reports/
const ROOT = path.resolve(__dirname, "..");
const CASES_PATH = path.resolve(ROOT, "rag-eval-cases.json");
const RESULTS_PATH = path.resolve(ROOT, "results", "rag-eval-results.json");
const REPORT_PATH = path.resolve(ROOT, "reports", "rag-eval-report.md");
...
async function main() {
  ...
  // Default:                results → scripts/results/rag-eval-results[-tag].json
  //                         report  → scripts/reports/rag-eval-report[-tag].md
  // --prompt-mode routed:   adds "-routedclass" suffix.
  // --out X:                results → scripts/results/reruns/rag-eval-results-X.json
  //                         report  → scripts/reports/reruns/rag-eval-report-X.md
  const resultsDir = args.out
    ? path.resolve(ROOT, "results", "reruns")
    : path.resolve(ROOT, "results");
  const reportsDir = args.out
    ? path.resolve(ROOT, "reports", "reruns")
    : path.resolve(ROOT, "reports");
  const suffix      = args.out
    ? `-${args.out}`
    : (promptMode === "routed" ? "-routedclass" : "");
  fs.mkdirSync(resultsDir, { recursive: true });
  fs.mkdirSync(reportsDir, { recursive: true });
  const resultsPath = path.resolve(resultsDir, `rag-eval-results${suffix}.json`);
  const reportPath  = path.resolve(reportsDir, `rag-eval-report${suffix}.md`);
