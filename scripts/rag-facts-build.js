...
const SEED = path.join(__dirname, "rag-seed.js");
const CASES = path.join(__dirname, "rag-eval-cases.json");

// ─── Load taskDefs from rag-seed.js ─────────────────────────────────────
...
function resolveFact(caseId, fact) {
  ...
  const { startText, endText } = fact;

  const firstStart = text.indexOf(startText);
  if (firstStart < 0) {
    throw new Error(
      `[${caseId}] startText not found in "${fact.taskTitle}":\n  ${JSON.stringify(startText)}`,
    );
  }
  const secondStart = text.indexOf(startText, firstStart + 1);
  if (secondStart >= 0) {
    throw new Error(
      `[${caseId}] startText is ambiguous (>=2 occurrences) in "${fact.taskTitle}":\n  ${JSON.stringify(
        startText,
      )}`,
    );
  }
  ...
  if (endIdx < 0) {
    throw new Error(
      `[${caseId}] endText not found after startText in "${fact.taskTitle}":\n  start=${JSON.stringify(
        startText,
      )}\n  end=${JSON.stringify(endText)}`,
    );
  }
  const end = endIdx + endText.length;

  const chunks = {};
  const taskChunks = chunksByTaskAndConfig.get(fact.taskTitle);
  for (const cfg of CHUNK_CONFIGS) {
    chunks[cfg.key] = classifyChunks(taskChunks[cfg.key], firstStart, end);
  }
