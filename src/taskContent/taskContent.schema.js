...
const taskContentSchema = new Schema(
  {
    ...
    // vector embedding of (task.title + task.description + plainText)
    // generated by text-embedding-3-small (1536 dims), used for semantic search in AI chat
    embedding: {
      type: [Number],
      default: undefined,
    },
    // true when content has changed but embedding hasn't been regenerated yet
    // set true on every content save, false after embedding succeeds
    // checked on GET to trigger async re-embed if a debounce timer was lost (e.g. server restart)
    embeddingStale: {
      type: Boolean,
      default: false,
    },
